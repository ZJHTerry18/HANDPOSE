...
def main(args):
    ...
    # get the train data
    train_dataset = ivg_HD_naive('/Extra/panzhiyu/CollectHD/', True)
    valid_dataset = ivg_HD_naive('/Extra/panzhiyu/CollectHD/', False)
    ...
    beta_collects = frange_cycle_linear(args.epochs, stop=0.5)
    if args.sample:
        number_s = 50
        get_samples, get_labels = sample_vis(model,checkpoint_file,number_s,True)
        get_samples = get_samples.reshape(number_s, 5,4)
        get_samples = get_samples.cpu().numpy()
        get_labels = get_labels.cpu().numpy()
        # import pdb;pdb.set_trace()
        vis(get_samples, get_labels,'vis_single/')
...
def train(train_queue, model, beta, cnn_optimizer, global_step, grad_scalar, warmup_iters, writer, logging): # temporally not using the grad scaling
    ...
    for step, data in enumerate(train_queue):
        cond, x, ang = data # TODO: cond can be used as the vae condition
        # cond as the 
        x = x.cuda()
        cond = cond.cuda()
        # change bit length
