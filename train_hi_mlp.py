...
from models.hi_vae_mlp_model import hi_vae
...
def main(args):
    ...
    model = hi_vae(20,args.latent_dims, args.MC_samples, args.IW_samples, args.threshold)
    model = model.cuda()
    logging.info('param size = %fM ', utils.count_parameters_in_M(model)) # counting the size

    if args.fast_adamax:
        if model.initialphase:
            cnn_optimizer = Adamax(model.outerloop.parameters(), args.learning_rate,
        ...
        if model.initialphase:
            cnn_optimizer = torch.optim.Adamax(model.outerloop.parameters(), args.learning_rate,
                                            weight_decay=args.weight_decay, eps=1e-3)
        else:
    ...
    if args.sample:
        number_s = 50
        get_samples = sample_vis(model,best_file,number_s) # best or checkpoint
        get_samples = get_samples.cpu().numpy()
        # import pdb;pdb.set_trace()
        vis(get_samples, 'vis_hi_50/')
    else:
        for epoch in range(init_epoch, args.epochs): # start training
            ...
            # beta = 1
            if model.initialphase == False and flag == 1:
                cnn_optimizer = torch.optim.Adamax(model.parameters(), args.learning_rate,
                                            weight_decay=args.weight_decay, eps=1e-3)
                logging.info('Changing mode into the associate training!')
                flag = 0
                # cnn_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                #     cnn_optimizer, float(args.epochs - args.warmup_epochs - 1), eta_min=args.learning_rate_min)
            ...
            if epoch % eval_freq == 0 or epoch == (args.epochs - 1):
                torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict(),
                                ...
                                'args': args, 'beta':model.beta.cpu(),'initphase':model.initialphase,
...
def sample_vis(model, checkpoint_file, num=10):
    checkpoint = torch.load(checkpoint_file, map_location='cpu')
    model.load_state_dict(checkpoint['state_dict'])
...
def train(train_queue, model, beta, cnn_optimizer, global_step, grad_scalar, warmup_iters, writer, logging): # temporally not using the grad scaling
    ...
    for step, data in enumerate(train_queue):
        ...
        if (global_step + 1) % 100 == 0:
            ...
            writer.add_scalar('train/re_loss', model.C, global_step)
...
def test(valid_queue, model, args, logging):
    ...
    for step, data in enumerate(valid_queue):
        ...
        with torch.no_grad():
            ...
            # norm [-pi, pi] to [-1,1]
            input_angle = org_angle / np.pi
            rec_loss = model.validate(input_angle, ang)

            vali_rel.update(rec_loss.item())
